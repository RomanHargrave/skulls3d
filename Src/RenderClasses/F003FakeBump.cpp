...
F003FakeBump::F003FakeBump(const Vec3f & v0, const Vec3f & v1, const Vec3f & v2,
...
void F003FakeBump::FillPoly(const Vec3f & normal, float *zbuffer)
{
	...
	while (y <= ymax  &&  y < m_viewport->m_y + m_viewport->m_h)
	{
		...
		while (ualphastep_x > 1 && mmp+1 < MIPMAPS_COUNT) {
			if (m_t->m_mipmaps[mmp].m_bitmap == NULL)
				break;
			mmp++;
			mmpe <<= 1;
			ualphastep_x >>= 1;
		}
		tbitmap = m_t->m_mipmaps[mmp].m_bitmap;
		if (m_tb != 0)
			tbumpmap = m_tb->m_mipmaps[mmp].m_bitmap;
		twidth = m_t->m_mipmaps[mmp].m_width;
		theight = m_t->m_mipmaps[mmp].m_height;
		...
		for ( ; x1 <= x2 ; x1++ )
		{
				...
				&& z < zbuffer[y*m_viewport->m_w+x1])
			{
				...
				if ( (actualx>-1.5) && (actualx<(int)twidth+0.5f) && (actualy>-1.5f) && (actualy<(int)theight+0.5f) )
				{
					u = (int)(floor(actualx));
					v = (int)(floor(actualy));

//					ASSERT(u>=-1 && u<=twidth-1 && v>=-1 && v<=theight-1);
					...
					if (tbumpmap != 0)
					{
						light.SetRotateXY((((char)tbumpmap[u+1+v])-((char)tbumpmap[u+v])) * 0.0031415926f,
						                  (((char)tbumpmap[u+v+twidth])-((char)tbumpmap[u+v])) * 0.0031415926f);
						normaldotproduct = frontvec.Dot(normal*light);
					}
					else
					{
						normaldotproduct = frontvec.Dot(normal);
					}

...
#include "R0062XDiffMipMapping.h"
...
} R0062TexCustomLineData;
int R0062TexCustomLineGetCurrentXThenStep(R0062TexCustomLineData & data);
void R0062TexInitCustomLine(R0062TexCustomLineData & data, const Vec3f & v0, const Vec3f & v1,
                           const Vec3f & t0, const Vec3f & t1, bool highest);
void R0062TexCustomLineJumpSteps(R0062TexCustomLineData & data, int steps);


R0062XDiffMipMapping::R0062XDiffMipMapping(Scene *scene, Camera *camera, ViewPort *viewport)
:R000Virtual(scene, camera, viewport)
{
}

void R0062XDiffMipMapping::Render()
{
	m_viewport->Clear();
...
void R0062XDiffMipMapping::FillPoly(const Vec3f & v0, const Vec3f & v1, const Vec3f & v2,
...
void R0062XDiffMipMapping::FillOrderedPoly(Vec3f v0, Vec3f v1, Vec3f v2,
									Vec3f t0, Vec3f t1, Vec3f t2, Texture *t)
{	
	...
	R0062TexCustomLineData bres1, bres2;
	R0062TexInitCustomLine(bres1, v0, v1, t0, t1, ratio1>ratio2);
	R0062TexInitCustomLine(bres2, v0, v2, t0, t2, ratio2>ratio1);
	...
	if (y < m_viewport->m_y)
	{
		...
		if (y1 >= m_viewport->m_y)
			...
			diff = y1 - y0;
		R0062TexCustomLineJumpSteps(bres1, diff);
		R0062TexCustomLineJumpSteps(bres2, diff);
		y += diff;
	}

	do {
		if (y >= ymax  &&  ymax != y2  &&  second_part == false) {
			R0062TexInitCustomLine(bres1, v1, v2, t1, t2, ratio1>ratio2);
			if (y < m_viewport->m_y)
			{
				R0062TexCustomLineJumpSteps(bres1, m_viewport->m_y - y);
				R0062TexCustomLineJumpSteps(bres2, m_viewport->m_y - y);
				y = m_viewport->m_y;
			}
			ymax = y2;
			second_part = true;
		}	
		x1 = R0062TexCustomLineGetCurrentXThenStep(bres1);
		x2 = R0062TexCustomLineGetCurrentXThenStep(bres2);

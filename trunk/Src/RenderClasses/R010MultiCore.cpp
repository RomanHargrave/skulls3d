...
R010MultiCore::R010MultiCore(Scene *scene, Camera *camera, ViewPort *viewport)
:R000Virtual(scene, camera, viewport)
{
	m_zbuffer = new float[m_viewport->m_w*m_viewport->m_h];
}

void R010MultiCore::Render()
{
	m_viewport->Clear();
	...
	for (int i=0 ; i<m_scene->m_meshCount ; i++)
	{
		...
		for (unsigned int s=0 ; s<mesh->m_stripcount ; s++)
		{
			...
			for (unsigned int i=2 ; i<strip->m_indexcount ; i++, v0=v1, v1=v2, tex_i+=3, triangle++)
			{
				...
				tp.t0 = t0; tp.t1 = t1; tp.t2 = t2;
				tp.neighb0visible = neighb0visible;
				tp.neighb1visible = neighb1visible;
				tp.neighb2visible = neighb2visible;
				tp.t = mesh->m_texture;
				tp.tb = mesh->m_bumpmap;
				tp.vp = m_viewport;
				tp.facenormal = worldviewnormals[triangle];
				tp.zbuffer = m_zbuffer;

				DWORD ok;
				if (thread_ids[0] == 0) {
					thread_ids[0] = CreateThread(NULL, 0, func, (LPVOID)&tp, 0, &ok);
				} else {
					if (thread_ids[1] != 0) {
						// wait for one thread to free
						DWORD id = WAIT_FAILED;
						DWORD error;
						while (id == WAIT_FAILED) {
							id = WaitForMultipleObjects(2,
													  thread_ids,
													  true,
													  INFINITE);
							if (id == WAIT_FAILED)
						...
						if (id >= WAIT_OBJECT_0 && id < WAIT_OBJECT_0+2) {
							if (id == WAIT_OBJECT_0)
								thread_ids[0] = thread_ids[1];
							thread_ids[1] = 0;
						} else if (id >= WAIT_ABANDONED_0 && id < WAIT_OBJECT_0+2) {
							if (id == WAIT_ABANDONED_0)
								thread_ids[0] = thread_ids[1];
							thread_ids[1] = 0;
						}
					}
					thread_ids[1] = CreateThread(NULL, 0, func, (LPVOID)&tp, 0, &ok);
				}
				// Fill poly
				/*
		...
		for (unsigned int s=0 ; s<mesh->m_fancount ; s++)
		{
			...
			for (unsigned int i=2 ; i<fan->m_indexcount ; i++, v1=v2, tex_i+=3, triangle++)
			{
				...
				tp.facenormal = worldviewnormals[triangle];
				tp.zbuffer = m_zbuffer;
				CreateThread(NULL, 0, func, &tp, 0, &id);
				/*
				F004Antialiasing::FillPoly(v0, v1, v2,
				                   t0, t1, t2,
				                   neighb0visible, neighb1visible, neighb2visible,
...
DWORD WINAPI func(LPVOID lpParameter)
{
	...
	F004Antialiasing::FillPoly(tp->v0, tp->v1, tp->v2,
	                   tp->t0, tp->t1, tp->t2,
	                   tp->neighb0visible, tp->neighb1visible, tp->neighb2visible,
					   tp->t, tp->tb, tp->vp,
					   tp->facenormal, tp->zbuffer);
	return 0;
}